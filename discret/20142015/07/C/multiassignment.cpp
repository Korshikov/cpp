...
struct graph
{
	struct edge 
	{
		edge(int from, int to, long long p, long long c, long 
long f, bool ret = false): from(from), to(to), p(p), c(c), f(f), 
ret(ret) {} 
	...
	long long min_cost(int s, int t)
	{
		...
		while (!(path = get_path(s, t)).empty())
		{
			...
			long long cost = 0;
			for (vector<edge*>::const_iterator it = 
...
//			cerr << "\n";
			ans += mn * cost;
			for (vector<edge*>::iterator it = path.begin(); 
it != path.end(); it++)
			{
				(*it)->f += mn;
				(*it)->rev->f -= mn;
	...
	pair<long long, vector<pair<int, int> > > assignment(int s, int 
t)
	{
		...
		for (int i = 0; i < n; i++)
		{
			if (i == s|| i == t)
				continue;
			for (vector<edge*>::const_iterator it = 
g[i].begin(); it != g[i].end(); it++)
			{
				edge *e = *it;
				if (e->to == s || e->to == t || 
e->remain() > 0 || e->ret)
					continue;
				ans.push_back(make_pair(e->from, 
e->to));
			}
		}
...
{	return out << "[" << e.from << "->" << e.to << ":: cost:" << e.p 
<< ", rem::" << e.remain() << "]";}
...
private:
	...
	{
		used[v] = true;
		for (set<int>::iterator it = g[v].begin(); it != 
g[v].end(); it++)
		{
			int to = *it;
...
int main()
{
	...
	ofstream out("multiassignment2.out");
	int n, k;
	in >> n >> k;
	graph g(2 * n + 2);
	for (int i = 0; i < n; i++)
	...
	pair<long long, vector<pair<int, int> > > ans = g.assignment(2 * 
n, 2* n + 1);
	int cnt = ans.second.size() / n;
	out << ans.first << "\n";
	graph2 g2(n,n);
	for (vector<pair<int, int> >::const_iterator it = 
ans.second.begin(); it != ans.second.end(); it++)
	{
		pair<int, int> e = *it;
	...
	for (int i = 0; i < cnt; i++)
	{
		...
		g2.clear(matching);
		for (vector<int>::const_iterator it = matching.begin(); 
it != matching.end(); it++)
			out << (*it + 1) << " ";
		out << "\n";
	}
